...
- Temporary HP
- AC
- Speed, if available in the play view
- Main attacks
- Actions
- Bonus Actions
- Reactions
...
- Switch between sections of the play view
...
- Hide empty content sections in the normal Play View. Use a calm empty state only when the character has no relevant playable content at all.
...
- Items and Notes are out of scope for this first Play View.
...
- Return to the Play View

### Secondary actions

- Expand for more detail, if available
- Move to the next or previous ability card, if the play view supports that pattern

### Empty / error / guest states

- If a detail entry is missing, the card should still open with a clear fallback such as “No additional detail available.”
- If the character has no abilities yet, the Play View should show a calm empty state and keep the layout usable.
...
- The sample character is strictly read-only in this first flow.
- On mobile, an ability, feature, or spell opens in a bottom sheet over the Play View.
...
- Default Play View sections are Overview, Actions, Bonus Actions, Reactions, Features, and Spells.
- Overview contains HP, temporary HP, AC, speed, active concentration, and trackable resources.
- Only show sections that have content.
- Items and Notes are not part of this first Play View.
