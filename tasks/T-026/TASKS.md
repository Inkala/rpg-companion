...
- [x] Confirm no implementation work starts in main checkout.
- [x] Confirm CharacterSheetV1 fields that will be updated, retained, confirmed, or rejected.
- [x] Confirm the all-12 SRD class, levels 1-5 cap is approved.
- [x] Confirm the T-026 worker owns no T-023 or shared orchestration files before editing.

## Slice 1: rules data and backend update contract

- [x] Add one canonical schema-validated SRD 5.1/2014 source for all 12 classes through level 5 and
  all SRD spells through spell level 3.
- [x] Add snapshot ID, SHA-256, source URLs, import date, transformation notes, and CC-BY-4.0
  attribution.
- [x] Add deterministic generation/checking for TypeScript and Go representations.
- [x] Add schema, checksum, and frontend/Go parity tests proving equivalent rules.
- [x] Do not add a live runtime rules API dependency.
- [x] Add rules-data tests for all 12 classes and transitions 1 to 2, 2 to 3, 3 to 4, and 4 to 5.
- [x] Add rules-data tests proving level 5 to 6 is unsupported.
- [x] Add rules-data tests for cantrips/spells through level 3, class-list membership, known and
  prepared progression, replacement limits, Wizard spellbook additions, Pact Magic, and
  Paladin/Ranger half-caster progression.
- [x] Add focused PATCH CORS tests for approved-origin preflight, unapproved origin, missing Origin
  with a credentialed unsafe request, credentials, and no wildcard.
- [x] Update `backend/internal/server/cors.go` to advertise PATCH only for approved-origin
  preflights while preserving existing protections.
- [x] Add backend tests for authenticated owner-scoped `PATCH /characters/{id}/level-up`.
- [x] Add backend tests for malformed ID/body and forbidden request fields.
- [x] Add backend tests proving foreign and unknown characters remain indistinguishable.
- [x] Add backend tests proving Party GM read-only access cannot level up.
- [x] Add backend tests for optimistic concurrency with `updatedAt`.
- [x] Add backend tests proving stale foreign requests do not reveal ownership or existence.
- [x] Add backend tests proving persisted owner, class, level, payload, and `updatedAt` are
  authoritative and loaded before transition validation.
- [x] Add backend tests rejecting submitted source/target level, class name, complete replacement
  payload, unknown fields, and illegal changes outside the level-up change set.
- [x] Add backend tests for exact public error codes and privacy-safe bodies.
- [x] Add backend tests for current level 0, current level 5, current level above 5, target level
  above 5, multiclass, unsupported class, and illegal level jump.
- [x] Add backend tests proving valid level-up persistence for each of the 12 classes for supported
  transitions.
- [x] Add backend tests for missing earlier class and spell prerequisites: collect safely or block
  without inventing/overwriting choices.
- [x] Add backend tests for exact CharacterSheetV1 provenance field mapping and bounds.
- [x] Add backend tests proving top-level columns and CharacterSheetV1 payload update in one
  transaction.
- [x] Add backend tests proving full resulting sheet validation before persistence.
- [x] Add backend tests proving repository/database failure causes no partial update.
- [x] Add backend tests proving Party membership link is preserved.
- [x] Implement the owner-scoped level-up update endpoint.
- [x] Implement repository transaction that locks and loads by owner ID before comparing
  `expectedUpdatedAt`.
- [x] Derive the permitted transition and resulting sheet server-side from persisted state,
  canonical rules, and bounded decisions.
- [x] Preserve every field outside the explicit level-up change set.
- [x] Implement server-side level-up invariants for the levels 1-5 bounded MVP.
- [x] Stop for review before Slice 2 if the backend/rules slice is split.
...
- [x] Stop for review before commit, push, PR, merge, deployment, or T-023 execution.
